...
read proxytype
	if [[ $proxytype != "socks" ]] && [[ $proxytype != "http" ]]
		then echo "Please enter either 'http' or 'socks'!"
			prxtp
		else echo "You will be using $proxytype proxy."
...
sed -i 's/#DefaultTasksMax=.*/DefaultTasksMax=300000/' /etc/systemd/system.conf
...
echo "РЎРѕР·РґР°РµРј СЃР»СѓР¶Р±Сѓ 3proxy.service"
...
echo "Creating random.sh, if possible"
...
echo -en "\033[37;1;41m Configuration finished, please restart \033[0m"
exit 0
